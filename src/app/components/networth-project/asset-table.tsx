...
export default function AssetTable({ selectedChains, address }: AssetTableProps) {
	...
	const fetchAssets = async (address: AssetTableProps["address"], selectedChains: AssetTableProps["selectedChains"]) => {
			...
			<tbody>
				{assets?.map((asset: any) => (
					<tr key={asset.chain + "-" + asset.token_address}>
						...
						<td className={asset.usd_price_24hr_percent_change < 0 ? "negative" : "positive"}>{asset.usd_price_24hr_percent_change?.toFixed(2)}%</td>
					</tr>
				))}
			</tbody>
		</table>
